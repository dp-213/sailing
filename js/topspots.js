(() => {
  ...
  // Try multiple data URLs to avoid path/cache issues
  ...
  (async function init(){
    try{
      ...
      const list = Array.isArray(data) ? data : (data && Array.isArray(data.spots) ? data.spots : []);
      allSpots=(list||[]).filter(s=>Number.isFinite(+s.lat)&&Number.isFinite(+s.lng)&&s.id&&s.name);
      // ensure all types enabled initially
