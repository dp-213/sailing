(() => {
  ...
  // Always fetch fresh (no SW cache); bump v= to bust caches after edits
  ...
  // Ensure proper sizing on first paint (fixes grey map when container was hidden)
  window.addEventListener('load', () => map.invalidateSize());

  // iOS/Safari: explicit default marker icons
  ...
  function render(spots) {
    ...
    spots.forEach(s => {
      ...
      li.addEventListener('click', (ev) => {
        if (ev.target.closest('.more')) return; // allow navigation
